...
CREATE TABLE IF NOT EXISTS `payments` (
  ...
  `proofImageUrl` VARCHAR(500) NULL COMMENT 'Required before payment confirmation',
  `confirmedBy` INT UNSIGNED NULL COMMENT 'Tournament organizer who confirmed the payment',
  `confirmedAt` DATETIME NULL,
  `refundedAt` DATETIME NULL,
  `refundProofImageUrl` VARCHAR(500) NULL COMMENT 'Required before payment refund',
  `createdAt` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `updatedAt` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
